...
import com.example.sptest.controller.User;
...
public class HttpCliTest {
    ...
    public void doGetTestWayOne() {
        ...
        try {
            ...
            params.append("name=" + URLEncoder.encode("山东济南高新区浪潮科技园", "utf-8"));
            params.append("&");
    ...
    public void doPostTestFour() {
        ...
        try {
            // 字符数据最好encoding以下;这样一来，某些特殊字符才能传过去(如:某人的名字就是“&”,不encoding的话,传不过去)
            params.append("name=" + URLEncoder.encode("&济南浪潮", "utf-8"));
            params.append("&");
            params.append("age=24");
        } catch (UnsupportedEncodingException e1) {
            e1.printStackTrace();
        ...
        try {
            ...
            System.out.println("响应状态为doPostTestFour:" + response.getStatusLine());
            if (responseEntity != null) {
                System.out.println("响应内容长度为:" + responseEntity.getContentLength());
                System.out.println("响应内容为:" + EntityUtils.toString(responseEntity));
            }
     ...
     */
    @Test
    public void doPostTestFive() {

        // 获得Http客户端(可以理解为:你得先有一个浏览器;注意:实际上HttpClient与浏览器是不一样的)
        CloseableHttpClient httpClient = HttpClientBuilder.create().build();

        // 创建Post请求
        HttpPost httpPost = new HttpPost("http://localhost:8080/doPostControllerFive");
        User user = new User();
        user.setName("潘晓婷");
        user.setAge(18);
        user.setGender("女");
